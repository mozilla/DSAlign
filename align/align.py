...
def align(triple):
    transcription_log, script, result = triple
...
def main():
    ...
    def enqueue_or_fail(prepare, audio, tlog, script, aligned, prefix=''):
        audio, tlog, script, aligned = prepare(audio), prepare(tlog), prepare(script), prepare(aligned)
        if aligned is not None and not args.force:
            fail(prefix + 'Alignment file "{}" already existing - use --force to overwrite'
                 .format(args.catalog, entry['aligned']))
        if (audio is None and tlog is None) or script is None:
            if args.ignore_missing:
                return
            if audio is None and tlog is None:
                if audio is None:
                    fail(prefix + 'Missing audio file "{}"'.format(args.catalog, entry['audio']))
                if tlog is None:
                    fail(prefix + 'Missing transcription log "{}"'.format(args.catalog, entry['tlog']))
            if script is None:
                fail(prefix + 'Missing script "{}"'.format(args.catalog, entry['audio']))
            if aligned is None:
                fail(prefix + 'Missing alignment file "{}"'.format(args.catalog, entry['aligned']))
    ...
    if (args.audio or args.tlog) and args.script and args.aligned and not args.catalog:
        enqueue_or_fail(check_path, args.audio, args.tlog, args.script, args.aligned)
    elif args.catalog:
        ...
        for entry in progress(catalog_entries, desc='Reading catalog'):
            enqueue_or_fail(lambda p: make_absolute(catalog_dir, p),
                            entry['audio'], entry['tlog'], entry['script'], entry['aligned'],
    ...
    for audio, tlog, script, aligned in to_prepare:
        ...
        if not path.exists(transcription_log) and not args.unaligned.endswith('.tlog'):
            ...
            segments, rate, audio_length = wavSplit.vad_segment_generator(args.unaligned, aggressiveness)
        ...
        if not path.exists(transcription_log):
    ...
    for _ in progress(pool.imap(align, to_align), desc='Aligning', total=1):
        pass
